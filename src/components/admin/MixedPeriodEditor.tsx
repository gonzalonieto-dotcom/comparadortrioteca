...
const MixedPeriodEditor = ({ periods, onChange }: Props) => {
  const add = () =>
      ...
      {periods.map((p, i) => (
        <div key={i} className="grid grid-cols-[60px_60px_80px_80px_auto] gap-2 items-end border rounded-lg p-3">
          ...
          <div>
            ...
            <Input type="number" value={p.to_year} onChange={(e) => update(i, { to_year: +e.target.value })} />
          </div>
          <div>
            <Label className="text-xs">TIN fijo %</Label>
            <Input type="number" step="0.01" value={p.fixed_tin ?? ""} onChange={(e) => update(i, { fixed_tin: e.target.value ? +e.target.value : null })} placeholder="-" />
          </div>
          <div>
